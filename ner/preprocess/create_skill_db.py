...
with open('../data/processed/skills_processed.json', 'r+') as f:
    SKILL_DB = json.load(f)
with open('../data/processed/token_dist.json', 'r+') as f:
    dist = json.load(f)
RELAX_PARAM = 0.2
new_skill_db = {}
for key in SKILL_DB:
    ...
    if skill_len == 2:
        ...
        last_count  = dist.get(last, 0)
        start_count = dist.get(start, 0)
        if last_count == 1:
            low_surface_form.append(last)

        # so sánh tỉ lệ khi có mẫu số > 0 để tránh ZeroDivisionError
        if last_count > 0 and (start_count / last_count) < RELAX_PARAM:
...
# save
with open('../data/processed/skill_db.json', 'w', encoding='utf-8') as f:
