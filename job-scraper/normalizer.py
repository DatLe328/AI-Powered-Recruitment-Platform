...
class Name_Normalizer():
    ...
    def __init__(self):
        self.stop_word_set = self.read_stop_word_set('data-norm/company_stop_words.csv')
        self.special_name_set = self.read_special_name_list('data-norm/special_companies.txt')
        self.acronym_original_set = self.read_acronym_list('data-norm/Suffix_Acronyms.csv')
    ...
    def read_special_name_list(self, filename):
        ...
        with open(filename, str('r'), encoding='utf-8') as source:
...
if __name__=="__main__":
    ...
    for company in samples:
        normalized = nn.normalize_company_name(company)
        print(f"Original: {company} → Normalized: {normalized}")